...
class Hue extends Emulator {
  ...
  addDevice(device) {
    if (Object.keys(this.devices).length >= 50) {
      debug("Max number of devices exceeded.");
      return function() {};
    }
    return (deviceName, buttonId) => {
      ...
      const h = crypto.createHash('sha1').update(deviceName).digest('hex').toUpperCase()
      this.devices[index] = {
        index: index,
        state: {
        ...
        uniqueId: `00:17:88:${h[32]}${h[33]}:${h[34]}${h[35]}:${h[36]}${h[37]}-${h[38]}${h[39]}`,
        buttonId: buttonId
      }
      return debug(`added device ${deviceName} as dimmable light with uniqueId ${this.devices[index].uniqueId}`);
    }
  }
  ...
  _changeState(device, state) {
    ...
    debug(`changing state for ${device.name}: ${JSON.stringify(state)}`)
    let that = this

    device.device.handler(state, function(result) {

      debug(`callback received with state ${result}`)
  ...
  _turnOn(device, buttonId) {
    debug(`device ${device.name} on`)
    return null
  }

  _turnOff(device) {
    debug(`device ${device.name} off`)
    return null
  }

  _getState(device) {
    debug(`device ${device.name} getState`)
    return true
  }

  _getBrightness(device) {
    debug(`device ${device.name} getBrightness`)
    return 127
  ...
  _setBrightness(device, dimLevel, buttonId) {
    debug(`device ${device.name} setBrightness to ${dimLevel}`)
    return null
  }
    ...
    emulator.get('/api/:userid/lights/:id', (req, res) => {
      ...
      if (this._authorizeUser(req.params["userid"], req, res)) {
        ...
        if (device) {
          ...
          debug(`device with id ${deviceId} not found`);
          return res.send(JSON.stringify({
            "error": {
              "type": 3,
              "address": req.path,
  ...
  _authorizeUser(username, req, res) {
    ...
    if ([].indexOf.call(this.users, username) >= 0) {
      ...
      debug(`Pairing is disabled and user ${username} was not found`);
